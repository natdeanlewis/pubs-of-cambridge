...
export default function Header({ pubs, visitedPubs, music, creditsMusic, setComplete, setRandomPub, setNearestPub, setMessage, setMusic, setCreditsMusic, playSound, map, INITIAL_MAP_SETTINGS, INITIAL_LATITUDE, INITIAL_LONGITUDE }) {
    ...
    const handleRandomPubClick = () => {
        ...
        if (pubs.length > 0 && pubs.length === visitedPubs.length) {
            ...
            if (creditsMusic) {
                ...
                disableInteractions();
                setTimeout(() => {
                    map.flyTo({center: [INITIAL_LONGITUDE, INITIAL_LATITUDE], zoom: 0, duration: 352000});
                }, 500);
            }
        } else {
